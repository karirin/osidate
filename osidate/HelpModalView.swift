    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                Button(action: {
                    if toggle {
                        viewModel.updateUserFlag(userId: viewModel.currentUserID!, userFlag: 1) { _ in }
                    }
                    viewModel.updateContact(userId: viewModel.currentUserID!, newContact: text){ success in
                        if success {
                            alertTitle = "送信完了"
