...
struct LoginBonus: Identifiable, Codable {
    let id: UUID
    let day: Int              // 連続ログイン日数
    let intimacyBonus: Int    // 親密度ボーナス
    let bonusType: BonusType  // ボーナスタイプ
    let receivedAt: Date      // 受取日時
    let description: String   // ボーナス説明
    
    enum BonusType: String, CaseIterable, Codable {
        case daily = "daily"           // 日常ボーナス
        case weekly = "weekly"         // 週間ボーナス
        case special = "special"       // 特別ボーナス
        case milestone = "milestone"   // マイルストーンボーナス
...
class LoginBonusManager: ObservableObject {
    ...
    private var isProcessingLogin = false  // 🌟 重複処理防止フラグ
